...
public class SpotServiceImpl implements SpotService {
    ...
    public boolean unlockSpot(Long userId, Long spotId) throws SQLException {
        ...
        try {
            ...
            // 낚시터 정보 조회 (가격 확인)
            if (spot == null) throw new RuntimeException("존재하지 않는 낚시터 입니다.");

            // 유저 조회
            if (user == null) throw new RuntimeException("존재하지 않는 회원입니다.");

            // 보유 골드 확인
            if (user.getGold() < spot.getUnlockPrice()) throw new RuntimeException("보유 골드가 부족합니다.");

            // 이미 잠금 해제했는지 확인
            if (userSpotDAO.existsByUserIdAndSpotId(conn, userId, spotId)) throw new RuntimeException("이미 잠금 해제된 낚시터입니다.");

            // 골드 차감하기
            if (!userDAO.updateGold(conn, userId, -spot.getUnlockPrice())) throw new RuntimeException("골드 차감에 실패했습니다");

            // 잠금 해제 기록 저장
            if (!userSpotDAO.insert(conn, userId, spotId)) throw new RuntimeException("잠금 해제 기록 저장에 실패했습니다.");
